...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Propaga erro real de Maritaca ao invés de mascarar
    ...
    // Se for erro de Maritaca, retorna com status 502 (Bad Gateway)
    if (errorMessage.includes("Maritaca")) {
      return NextResponse.json(
        { message: `Erro ao processar com IA: ${errorMessage}. Verifique a chave de API.` },
        { status: 502 }
      );
    }
    
    // Erro genérico: pipeline falhou
